...
Wow, you stepped up ! Now you can drink a Grimbergen.
In the Grimbergen, the flag is changed. Get the beer's default flag.
...
sanitize=re.compile(r'(?:import|locals|exec|eval|join|format|replace|translate|try|except|with|content|frame|back|os|sys|dict|base|subclasses|open|builtins)').sub
...
def verify_flag(flag):
    ...
    if true_flag==flag:
        print("Well done, you managed to get the flag !\nLet's have a better beer !\n\nNext time you'll come, you'll have access to this new beer with this flag as password to the user pybeer3")
    else:
...
class beer:
    ...
    def __init__(self,empty=False,name="beautiful Maximator", flag="H2G2{y0u_knOw_HOW_cL4S5es_w0rk_oN_pyth0n}"):
        self._empty=empty
...
grimbergen=beer(name="Grimbergen", flag="It's not the default flag. Look further.")
...
def main():
    ...
    def sandbox():
        ...
        exec_code({"grimbergen":grimbergen,"verify_flag":verify_flag})
